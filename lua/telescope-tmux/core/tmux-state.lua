  ...
  if not __sessions_by_id[session_id].windows then
	...
	for window_id, window_name in pairs(windows) do
		if not __sessions_by_id[session_id].windows[window_id] then
			__sessions_by_id[session_id].windows[window_id] = { window_id = window_id, window_name = window_name, last_used = 0 }
		else
			__sessions_by_id[session_id].windows[window_id].window_name = window_name -- force update the window name
		end
	end
end
...
local __add_session_to_session_cache = function(session_id, session_details)
    ...
    __add_window_data_to_session_id(session_id, session_details.windows)
		...
		__sessions_by_id[session_id] = { session_name = session_details.session_name, last_used = 0, session_id = session_id }
    __add_window_data_to_session_id(session_id, session_details.windows)
	end
